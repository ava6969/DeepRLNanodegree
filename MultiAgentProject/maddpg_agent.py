...
# LEARN_EVERY = 1         # how often to replay stored memories
# REPLAY_PASSES = 5      # how many times to repeat when replaying
# EPSILON = 5.0           # multiplicative factor for the noise
# EPSILON_DECAY = 1e-2    # epislon decay rate
# EPSILON_MIN = 0
...
#         self.epsilon = EPSILON        # noise decay
...
#     def step(self, state, action, reward, next_state, time, done, agent_num):
#         """Save experience in replay memory, and use random sample from buffer to learn."""
#         # Save experience / reward
#         self.memory.add(state, action, reward, next_state, done)
        
#         # Learn, if enough samples are available in memory
#         if len(self.memory) > BATCH_SIZE and time%LEARN_EVERY == 0:
#             for rp in range(REPLAY_PASSES):
#                 experiences = self.memory.sample()
#                 self.learn(experiences, GAMMA, agent_num)

#     def act(self, states, add_noise=True):
#         """Returns actions for given state (for two agents) as per current policy."""
#         states = torch.from_numpy(states).float().to(device)
#         actions_array = np.zeros((self.num_agents, self.action_size))
#         self.actor_local.eval()
#         with torch.no_grad():
#             for agent_num, state in enumerate(states):
#                 action = self.actor_local(state).cpu().data.numpy()
#                 actions_array[agent_num,:] = action
#         self.actor_local.train()
#         if add_noise:
#             actions_array += self.noise.sample() * self.epsilon
#         return np.clip(actions_array, -1, 1)
...
#             experiences (Tuple[torch.Tensor]): tuple of (s, a, r, s', done) tuples 
#             gamma (float): discount factor
#             agent_num: agent number, which determines how the agent constructs the next action vector
#         """
#         states, actions, rewards, next_states, dones = experiences

#         # ---------------------------- update critic ---------------------------- #
#         # Get predicted next-state actions and Q values from target models
#         actions_next_temp = self.actor_target(next_states)
#         if agent_num == 0:
#             actions_next = torch.cat((actions_next_temp, actions[:,2:]), dim = 1)
#         else:
#             actions_next = torch.cat((actions[:,:2], actions_next_temp), dim = 1)
...
#         # ---------------------------- update actor ---------------------------- #
#         # Compute actor loss
#         actions_pred_temp = self.actor_local(states)
#         if agent_num == 0:
#             actions_pred = torch.cat((actions_pred_temp, actions[:,2:]), dim = 1)
#         else:
#             actions_pred = torch.cat((actions[:,:2], actions_pred_temp), dim = 1)
