...
class Agent():
    ...
    def __init__(self, state_size, action_size, num_agents, random_seed):
        ...
        # Replay memory
        self.memory = ReplayBuffer(action_size, h.BUFFER_SIZE, h.BATCH_SIZE, random_seed)
    ...
    def step(self, state, action, reward, next_state, done, agent_number):
        ...
        if len(self.memory) > h.BATCH_SIZE and self.timestep % h.LEARN_EVERY == 0:
                for _ in range(h.LEARN_PASSES):
                    ...
                    self.learn(experiences, h.GAMMA, agent_number)
...
class OUNoise:
    ...
    def __init__(self, size, seed, mu=0.0, theta=h.OU_THETA, sigma=h.OU_SIGMA):
